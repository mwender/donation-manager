jQuery( function($){
	...
	// Updating `show_in_results` for Orphaned Contacts
	$(document).on('change','.show-in-results',function(){
		contactID = $(this).closest('tr').attr('data-id');
		show_in_results = $(this).is(':checked');
		if( true == show_in_results ){
			show_in_results = 1;
		} else {
			show_in_results = 0;
		}
		updateContact(contactID,'show_in_results',show_in_results);
	});
	...
	$('#form-unsubscribe').submit(function(e){
		e.preventDefault();
		updateEmail( 'unsubscribe' );
	});

	// Subscribe emails
	$('#form-subscribe').submit(function(e){
		e.preventDefault();
		updateEmail( 'subscribe' );
	});
	...
	function addContact(){
		...
		if( $('#contact-show_in_results').is(':checked') ){
			var show_in_results = 1;
		} else {
			var show_in_results = 0;
		}
	...
	function testPcode(){
		...
		var priority = $('#test-priority').val();
	...
	function deleteContact( contactID ){
		var data = {
			'action': 'orphaned_utilities_ajax',
			'cb_action': 'delete_contact',
			'ID': contactID
		};
		$.post(wpvars.ajax_url,data,function(response){
			console.log(response.success);
			$('#contact_' + response.ID ).fadeOut();
		});
	}
	...
	function updateContact( id, field, value ){
		var data = {
			...
			'value': value,
			'action': 'orphaned_utilities_ajax',
			'cb_action': 'update_contact'
		};
		$.post(wpvars.ajax_url, data, function(response){
			backgroundColor = $('#contact_' + response.ID ).css('backgroundColor');
			$('#contact_' + response.ID ).animate({
				backgroundColor: "#d1e6cf"
			},300).animate({
	...
	/**
	 * Subscribes/Unsubscribes an email
	 *
	 * @method     updateEmail
	 */
	function updateEmail( action ){
		...
		$.post( wpvars.ajax_url, data, function(response){
			$('#output-' + action ).html(dismissButton + response.output);
		});
	}

	// BEGIN Datatables

	// Orphaned Donations Report
	var table = $('#orphaned-donations').DataTable({
		fixedHeader: true,
		processing: true,
		serverSide: true,
		dom: 'l<"date_range"><"donation_priority">f<"filterby">rtip',
		ajax: {
			url: wpvars.ajax_url,
			type: 'POST',
			data: function( d ){
	...
	$('div.filterby').html('Filter by: <select name="filterby" id="filterby"><option value="email_address">Email</option><option value="zipcode">Zip Code</option><option value="store_name">Store Name</option></select>');
    ...
    // Providers Report
    var tablePickUpProviders = $( '#orphaned-donation-pickup-providers').DataTable({
 		fixedHeader: true,
		processing: true,
		serverSide: true,
		dom: 'lf<"searchfield">rtip',
		ajax: {
			url: wpvars.ajax_url,
			type: 'POST',
			data: function( d ){
				d.action = 'query_orphaned_donation_pickup_providers';
		...
		columnDefs: [
			{ name: 'id', data: 'id', className: 'dt-body-right', orderable: false, targets: 0 },
			{ name: 'store_name', data: 'store_name', orderable: true, targets: 1 },
			{ name: 'email_address', data: 'email_address', orderable: false, targets: 2 },
			{ name: 'zipcode', data: 'zipcode', className: 'dt-body-right', orderable: true, targets: 3 },
			{ name: 'priority', data: 'priority', orderable: true, targets: 4 },
			{ name: 'receive_emails', data: 'receive_emails', className: 'dt-body-center', orderable: false, targets: 5 }
		]
    });
    $('div.searchfield')
	    .html('Search field: <select name="searchfield" id="searchfield"><option value="email_address">Email</option><option value="store_name">Store Name</option><option value="zipcode">Zip Code</option></select>')
	    .on( 'change', function(){
	    	var searchStr = $( '#orphaned-donation-pickup-providers_filter input[type="search"]' ).val();
	    	if( '' != searchStr )
		    	tablePickUpProviders.draw();
    });

    // Returned AJAX object
    if( true == wpvars.debug ){
	    tablePickUpProviders.on( 'xhr.dt', function(e,settings,json){
	        console.log( '[DT] Ajax event occured:' );
	        console.log(json);
	    });
	  }
