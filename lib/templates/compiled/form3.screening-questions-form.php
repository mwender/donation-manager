<?php
 function lcr57b3511db64f7sec($cx, $v, $bp, $in, $each, $cb, $else = null) {
  $push = ($in !== $v) || $each;
  ...
  if (($loop && $isAry) || $isTrav) {
   ...
   foreach ($v as $index => $raw) {
    ...
    if (isset($bp[0])) {
     $raw = lcr57b3511db64f7m($cx, $raw, array($bp[0] => $raw));
    }
    if (isset($bp[1])) {
     $raw = lcr57b3511db64f7m($cx, $raw, array($bp[1] => $cx['sp_vars']['index']));
    }
    $ret[] = $cb($cx, $raw);
   }
   if ($cx['flags']['spvar']) {
 ...
 function lcr57b3511db64f7m($cx, $a, $b) {
  if (is_array($b)) {
   if ($a === null) {
    return $b;
   } else if (is_array($a)) {
...
return function ($in = null, $options = null) {
    ...
    ob_start();echo '<form action="" method="post" name="screening-questions">
	<table class="table table-striped"><colgroup><col style="width: 70%;" /><col style="width: 15%" /><col style="width: 15%" /></colgroup>

',lcr57b3511db64f7sec($cx, ((is_array($in) && isset($in['questions'])) ? $in['questions'] : null), null, $in, true, function($cx, $in) {echo '	<tr>
