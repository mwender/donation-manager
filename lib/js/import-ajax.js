jQuery(document).ready(function($){
	...
	function delete_donations(){
		...
		$('#delete-donations-status').html( 'Deleting...Please wait.' );
		$.post( ajax_object.ajax_url, data, function(response){
			if( 0 < response.donation_count ){
				$('#delete-donations-status').html( '<strong>' + response.numberposts + '</strong> donations of <strong>' + response.donation_count + '</strong> deleted.' );
				delete_donations();
			} else {
